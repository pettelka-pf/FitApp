...
public class CalorieDatabase implements CalorieRepository {
    ...
    @Override
    public int getEatenOnDate(int userId, LocalDate date) throws SQLException {
        return queryInt(
                "SELECT SUM(calories) FROM meals WHERE user_id = ? AND date = ?",
                userId,
                date
    ...
    public int getBurnedToday(int userId, LocalDate date) throws SQLException {
        return queryInt(
                "SELECT SUM(calories_burned) FROM exercises WHERE user_id = ? AND date = ?",
                userId,
                date
        );
    }
     ...
     *
     * SUM() liefert NULL, wenn es fuer den Tag keine Zeile gibt.
     * getDouble() liefert in diesem Fall 0, deshalb ist kein COALESCE notwendig.
     *
     * exercises.calories ist DOUBLE PRECISION, darum wird gerundet
     * statt abgeschnitten.
     */
